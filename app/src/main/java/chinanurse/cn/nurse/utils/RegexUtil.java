...
/**
 * 正则工具类 提供验证中文名、邮箱、手机号、电话号码、身份证号码、数字等方法
 */
...
//        String regex = "(^(13[0-9]|15[0-9]|18[0-9]|17[0-9]|147)\\d{8}$)|(^(0\\d{2})-(\\d{8})$)|(^(0\\d{3})-(\\d{7})$)|(^(0\\d{2})-(\\d{8})-(\\d+)$)|(^(0\\d{3})-(\\d{7})-(\\d+)$)";
     ...
     * @return
     */
    public static boolean checkNumberAndzifu(String character) {
     ...
     * @return
     */
    public static boolean checkText(String character) {
        String regex = "!|！|@|◎|#|＃|(\\$)|￥|%|％|(\\^)|……|(\\&)|※|(\\*)|×|(\\()|(|(\\))|)|_|——|(\\+)|＋|(\\|)|§";
        return Pattern.matches(regex, character);
    }
    ...
    public static boolean checkname(String character) {
        String regex = "([\\u4e00-\\u9fa5]{2,})|([A-Za-z0-9 ]{4,})";
        return Pattern.matches(regex, character);
    }
}