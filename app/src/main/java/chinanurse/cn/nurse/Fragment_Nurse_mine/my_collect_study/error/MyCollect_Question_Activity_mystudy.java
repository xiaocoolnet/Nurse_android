package chinanurse.cn.nurse.Fragment_Nurse_mine.my_collect_study.error;
...
import chinanurse.cn.nurse.Fragment_Nurse_mine.my_collect_study.error.adapter.MyCollectQuestion_Adapter_mystudy;
import chinanurse.cn.nurse.HttpConn.HttpConnect;
import chinanurse.cn.nurse.HttpConn.request.StudyRequest;
import chinanurse.cn.nurse.MinePage.MyStudy.MineStudt_question_error_Bean;
import chinanurse.cn.nurse.R;
import chinanurse.cn.nurse.Fragment_Nurse_mine.my_collect_study.error.bean.Question_hashmap_data_error;
 ...
 */
public class MyCollect_Question_Activity_mystudy extends Activity implements View.OnClickListener {
...
//    private List<MyCollectQuestion_Bean.DataBean> answerlistbean_over = new ArrayList<>();
    ...
    private List<MineStudt_question_error_Bean.DataBean> listdata= Question_hashmap_data_error.MyCollectList;
...
//    public static int gradview_position;
    ...
    private MyCollectQuestion_Adapter_mystudy pagerAdapter;
    private int pageScore, allpageNum, errortopicNums, errortopicNums1;//分数，页数（题数）、错题数

    private UserBean user;
    private Intent intent;
    private MineStudt_question_error_Bean.DataBean dailydata;
                    ...
                    if (msg.obj != null) {
                        ...
                        try {
                            ...
                            if ("success".equals(status)) {
                                Toast.makeText(MyCollect_Question_Activity_mystudy.this, R.string.question_collect_succcess, Toast.LENGTH_SHORT).show();
                                if (collectTag == null) {
                                    return;
                                }
...
//                                collectTag.setBackgroundResource(R.mipmap.btn_collet);
                                Toast.makeText(MyCollect_Question_Activity_mystudy.this, R.string.question_cancel_collect, Toast.LENGTH_SHORT).show();
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                        ...
                        try {
                            ...
                            if ("success".equals(obj.optString("status"))) {
                                if("had".equals(obj.optString("data"))){
                                    if (HttpConnect.isConnnected(MyCollect_Question_Activity_mystudy.this)) {
                                        new StudyRequest(MyCollect_Question_Activity_mystudy.this, collecthandler).DELLCOLLEXT(user.getUserid(), listdata.get(ViewPosition).getQuestionid(),
                                                "2", CANCELCOLLECT);
//
                                    } else {
                                        Toast.makeText(MyCollect_Question_Activity_mystudy.this, R.string.net_erroy, Toast.LENGTH_SHORT).show();
                                    }
                                }
                            }else if("error".equals(obj.optString("status"))){
                                if("no".equals(obj.optString("data"))){
                                    if (listdata.get(ViewPosition).getPost_description() != null&&listdata.get(ViewPosition).getPost_description().length() > 0){
                                        decrisp = listdata.get(ViewPosition).getPost_description().toString();
                                    }else{
                                        decrisp = "";
                                    }
                                    if (HttpConnect.isConnnected(MyCollect_Question_Activity_mystudy.this)) {
                                        Log.i("试题收藏.....", "============>" + listdata.get(ViewPosition).getPost_title());
                                        new StudyRequest(MyCollect_Question_Activity_mystudy.this, collecthandler).COLLEXT(user.getUserid(), listdata.get(ViewPosition).getQuestionid(), "2",
                                                listdata.get(ViewPosition).getPost_title(), decrisp, SETCOLLECT);
                                    } else {
                                        Toast.makeText(MyCollect_Question_Activity_mystudy.this, R.string.net_erroy, Toast.LENGTH_SHORT).show();
                                    }
...
//                    answerlistbean_over.add(dailydata);
                    ...
                    pagerAdapter = new MyCollectQuestion_Adapter_mystudy(
                            MyCollect_Question_Activity_mystudy.this, viewItems,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent = getIntent();
        //获取当前题目
        dailydata = (MineStudt_question_error_Bean.DataBean) intent.getSerializableExtra("collquestion");
        //获取总列表
...
//        }
    ...
    public void isCollect() {
        ...
        if (HttpConnect.isConnnected(mactivity)) {
            new StudyRequest(mactivity, hanlder).ISCOLLECT(user.getUserid(), listdata.get(ViewPosition).getQuestionid()/**题目ID**/, "2", ISCOLLECT);
        }else{
            Toast.makeText(mactivity, R.string.net_erroy, Toast.LENGTH_SHORT).show();
        }
        if(collectTag==null){
        ...
        collectTag.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (HttpConnect.isConnnected(mactivity)) {
                    new StudyRequest(mactivity, collecthandler).ISCOLLECT(user.getUserid(), listdata.get(ViewPosition).getQuestionid()/**题目ID**/, "2", ISCOLLECTONE);
                } else {
                    Toast.makeText(mactivity, R.string.net_erroy, Toast.LENGTH_SHORT).show();
                }
            }
