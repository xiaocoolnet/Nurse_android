 ...
 * 我的收藏---试题适配器
 */
public class MyCollectQuestion_Adapter_mystudy extends PagerAdapter implements OnClickListener {

    MyCollect_Question_Activity_mystudy mContext;
    // 传递过来的页面view的集合
    List<View> viewItems;
    // 每个item的页面view
    View convertView;
    // 传递过来的所有数据
    List<MineStudt_question_error_Bean.DataBean> answerItems;
    //错题容器
    ...
    public MyCollectQuestion_Adapter_mystudy(MyCollect_Question_Activity_mystudy context, List<View> viewItems, List<MineStudt_question_error_Bean.DataBean> dataItems, String questionType, String questionNum, int mposition) {
        mContext = context;
...
//            if(answerItems.get(position).getPost_title().toString()==null){
//                holder.question_title.setText(answerItems.get(position).getPost_title().toString()+"");
//            }else{
        holder.question_title.setText(answerItems.get(position).getPost_title().toString());
//            }
        holder.adapter = new MyCollect_question_answer_anapdter_error(mContext, answerItems, position);
        ...
        holder.listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int lv_position, long id) {
                //得到正确答案
                for (int i = 0; i < answerItems.get(position).getAnswers().size(); i++) {
                    if ("1".equals(answerItems.get(position).getAnswers().get(i).getIsanswer())) {
                        holder.correct_answer = holder.option[i];
                        RightPosition = i;
                        //得到正确答案的描述
                        answer_description = answerItems.get(position).getAnswers().toString();
                        //是个null，需要接口正确了再改回来
//                            answer_difficulty = Integer.valueOf(answerItems.get(position).getPost_difficulty().toString());
                ...
                if (v != null) {
                    holder.tv_answer_list = (TextView) v.findViewById(R.id.tv_answer_list);
                    ...
                    holder.tv_answer_list.setTextColor(mContext.getResources().getColor(R.color.whilte));
...
//                    holder.answer_score = holder.question_score.getText().toString();
                ...
                holder.tv_answer_list = (TextView) view.findViewById(R.id.tv_answer_list);
                ...
                //判断题目的对错
                if ("1".equals(answerItems.get(position).getAnswers().get(lv_position).getIsanswer())) {
                ...
                } else if ("0".equals(answerItems.get(position).getAnswers().get(lv_position).getIsanswer().toString())) {
                    ...
                    answer_list.setAnswer_title(answerItems.get(position).getAnswers().get(lv_position).getTitle() + "");
                    answer_list.setIsanswer(answerItems.get(position).getAnswers().get(lv_position).getIsanswer() + "");
                ...
                questionInfo.setQuestionId(answerItems.get(position).getQuestionid());
...
*/
    ...
    public class ViewHolder {
        ...
        private MyCollect_question_answer_anapdter_error adapter;
        private TextView present_topic, question_title, question_score, question_test, tv_answer_list, question_option;
