package chinanurse.cn.nurse.Fragment_Mine.mine_info;
...
public class MyInFo_Update extends Activity implements View.OnClickListener{
                    ...
                    if (msg.obj != null){
                        ...
                        try {
                            ...
                            if ("success".equals(status)) {
                                if ("Name repetition".equals(data)) {
                                    Toast.makeText(mactivity, "名称重复，请重新填写", Toast.LENGTH_SHORT).show();
                                } else {
            ...
            case R.id.myinfo_update_save:
                if ("name".equals(updatetype)) {
                    ...
                    if (ispHONE()){
                        ...
                        Toast.makeText(mactivity,"用户名要大于4个字母或者2个汉字",Toast.LENGTH_SHORT).show();
                    }
                    ...
                    if (isCanGetCodeemile()) {
                        ...
                        Toast.makeText(mactivity,"请输入正确的邮箱格式",Toast.LENGTH_SHORT).show();
                    }
                } else if ("school".equals(updatetype)) {
                    et_content = update_content.getText().toString();
                    if (HttpConnect.isConnnected(mactivity)) {
    ...
    private Boolean ispHONE() {
        if (et_content == null || "".equals(et_content)) {
            Toast.makeText(mactivity,"请输入用户名称",Toast.LENGTH_SHORT).show();
        }else {
            if (!RegexUtil.checkname(et_content)) {
                return false;
            }
        }
        return true;
    ...
    private Boolean isCanGetCodeemile() {
        if (et_content == null || "".equals(et_content)) {
            Toast.makeText(mactivity,"请输入邮箱",Toast.LENGTH_SHORT).show();
        }else {
            if (!RegexUtil.checkEmail(et_content)) {
                return false;
            }
        }
        return true;
...
//            alert.show();
    ...
    protected void onResume() {
        super.onResume();
        StatService.onPageStart(this, "个人信息编辑");
    }
    ...
    protected void onPause() {
        ...
        StatService.onPageEnd(this, "个人信息编辑");
    }
}
