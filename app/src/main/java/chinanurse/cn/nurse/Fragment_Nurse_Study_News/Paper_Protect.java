...
public class Paper_Protect extends AppCompatActivity {
    private static final int GETNUM = 11;
    private static final int FIRSTPAGELISTON = 22;
    /**
     * 推送消息发送广播
     */
    ...
    private String channelid = "95";
    //  新建的容器 但是容器的内容暂时延用新闻页的
    private ArrayList<FirstPageNews.DataBean> fndlist = new ArrayList<>();
    private ArrayList<FirstPageNews.DataBean> fndlistONE = new ArrayList<>();
                    ...
                    if (result != null) {
                        ...
                        try {
                            ...
                            if ("success".equals(json.optString("status"))){
                                gson = new Gson();
                                fndbean = gson.fromJson(result, FirstPageNews.class);
                                fndlistONE.addAll(fndbean.getData());
                                editor.putString("fndlist",String.valueOf(fndlistONE.size()));
                                Log.e("list_mine_recruit","-------------->"+fndlistONE.size());
                                editor.commit();
                            }else{
                                if ("end".equals(data)){
                                    Toast.makeText(mactivity,"无更新数据",Toast.LENGTH_SHORT).show();
    ...
    private void initData() {
        ...
        sp = getSharedPreferences("nursenum", Context.MODE_PRIVATE);
        editor=sp.edit();
...
//        pulllist.doPullRefreshing(true, 500);
    ...
    private void getnewslistother() {
        if (HttpConnect.isConnnected(mactivity)){
            ...
            dialogpgd.show();
            new StudyRequest(Paper_Protect.this, handler).getNewsListother(channelid,String.valueOf(page),FIRSTPAGELIST);
        }else{
...
//        gettime();
    ...
    private void inithdata() {
        if (HttpConnect.isConnnected(mactivity)){
            ...
            dialogpgd.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            dialogpgd.show();
            new StudyRequest(Paper_Protect.this, handler).getNewsList(channelid,FIRSTPAGELISTON);
        }else{
            Toast.makeText(mactivity, R.string.net_erroy, Toast.LENGTH_SHORT).show();
            ril_shibai.setVisibility(View.VISIBLE);
            ril_list.setVisibility(View.GONE);
...
//            if (HttpConnect.isConnnected(mactivity)){
//                new StudyRequest(Paper_Protect.this, handler).getNewslist_count(user.getUserid(),String.valueOf(timecurrentTimeMillis),GETNUM);
//            }else{
//                Toast.makeText(mactivity, R.string.net_erroy, Toast.LENGTH_SHORT).show();
//
//            }
